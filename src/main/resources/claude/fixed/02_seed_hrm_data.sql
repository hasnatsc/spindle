...
FROM hrm_employees e WHERE e.employee_code = 'EMP-0001' AND e.organization_id = 1
  AND NOT EXISTS (SELECT 1 FROM hrm_employee_addresses a WHERE a.employee_id = e.id AND a.address_type = 'PRESENT');
...
FROM hrm_employees e WHERE e.employee_code = 'EMP-0001' AND e.organization_id = 1
  AND NOT EXISTS (SELECT 1 FROM hrm_employee_salaries s WHERE s.employee_id = e.id AND s.is_current = true);
