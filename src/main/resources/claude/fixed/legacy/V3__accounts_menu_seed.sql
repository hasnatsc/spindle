...
SELECT v.* FROM (VALUES
  ...
  ('M_ACC_MASTERS',          'Masters',                    '#',
   'fas fa-layer-group',
   (SELECT id FROM app_menus WHERE menu_code='M_ACCOUNTS'), 10,'GROUP','ACCOUNTS',NULL,TRUE, TRUE, FALSE),

  ('M_ACC_COA',              'Chart of Accounts',          '/accounts/chart',
   'fas fa-sitemap',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 10,'LEAF','ACCOUNTS','accounts.coa.view',TRUE, TRUE, FALSE),

  ('M_ACC_SUB',              'Sub-Ledger Accounts',        '/accounts/sub-accounts',
   'fas fa-list-tree',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 20,'LEAF','ACCOUNTS','accounts.subledger.view',TRUE, TRUE, FALSE),

  ('M_ACC_BANK_ACCTS',       'Bank Accounts',              '/accounts/bank-accounts',
   'fas fa-building-columns',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 30,'LEAF','ACCOUNTS','accounts.bank.view',TRUE, TRUE, FALSE),

  ('M_ACC_CASH_ACCTS',       'Cash Accounts',              '/accounts/cash-accounts',
   'fas fa-vault',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 40,'LEAF','ACCOUNTS','accounts.cash.view',TRUE, TRUE, FALSE),

  ('M_ACC_CUSTOMERS',        'Customer Accounts',          '/accounts/customer-accounts',
   'fas fa-user-tie',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 50,'LEAF','ACCOUNTS','accounts.customer.view',TRUE, TRUE, FALSE),

  ('M_ACC_SUPPLIERS',        'Supplier Accounts',          '/accounts/supplier-accounts',
   'fas fa-truck',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 60,'LEAF','ACCOUNTS','accounts.supplier.view',TRUE, TRUE, FALSE),

  ('M_ACC_EMPLOYEES',        'Employee Accounts',          '/accounts/employee-accounts',
   'fas fa-id-badge',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 70,'LEAF','ACCOUNTS','accounts.employee.view',TRUE, TRUE, FALSE),

  ('M_ACC_PERIODS',          'Accounting Periods',         '/accounts/periods',
   'fas fa-calendar-days',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_MASTERS'), 80,'LEAF','ACCOUNTS','accounts.period.view',TRUE, TRUE, FALSE),

  -- ── TRANSACTIONS GROUP ───────────────────────────────────────────────────────
  ('M_ACC_TX',               'Transactions',               '#',
   'fas fa-receipt',
   (SELECT id FROM app_menus WHERE menu_code='M_ACCOUNTS'), 20,'GROUP','ACCOUNTS',NULL,TRUE, TRUE, FALSE),

  ('M_ACC_JOURNAL',          'Journal Voucher',            '/accounts/journals',
   'fas fa-book-open',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 10,'LEAF','ACCOUNTS','accounts.journal.view',TRUE, TRUE, FALSE),

  ('M_ACC_PAYMENT',          'Payment Voucher',            '/accounts/payment-vouchers',
   'fas fa-money-bill-transfer',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 20,'LEAF','ACCOUNTS','accounts.payment.view',TRUE, TRUE, FALSE),

  ('M_ACC_RECEIPT',          'Receipt Voucher',            '/accounts/receipt-vouchers',
   'fas fa-money-bill-wave',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 30,'LEAF','ACCOUNTS','accounts.receipt.view',TRUE, TRUE, FALSE),

  ('M_ACC_CONTRA',           'Contra Voucher',             '/accounts/contra-vouchers',
   'fas fa-right-left',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 40,'LEAF','ACCOUNTS','accounts.contra.view',TRUE, TRUE, FALSE),

  ('M_ACC_DEBIT_NOTE',       'Debit Note',                 '/accounts/debit-notes',
   'fas fa-file-circle-minus',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 50,'LEAF','ACCOUNTS','accounts.debitnote.view',TRUE, TRUE, FALSE),

  ('M_ACC_CREDIT_NOTE',      'Credit Note',                '/accounts/credit-notes',
   'fas fa-file-circle-plus',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 60,'LEAF','ACCOUNTS','accounts.creditnote.view',TRUE, TRUE, FALSE),

  ('M_ACC_OPENING_BAL',      'Opening Balances',           '/accounts/opening-balances',
   'fas fa-scale-balanced',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_TX'), 70,'LEAF','ACCOUNTS','accounts.openingbal.view',TRUE, TRUE, FALSE),

  -- ── CONFIGURATION GROUP ──────────────────────────────────────────────────────
  ('M_ACC_CONFIG',           'Configuration',              '#',
   'fas fa-sliders',
   (SELECT id FROM app_menus WHERE menu_code='M_ACCOUNTS'), 30,'GROUP','ACCOUNTS',NULL,TRUE, TRUE, FALSE),

  ('M_ACC_MAPPING',          'Accounts Mapping',           '/accounts/mapping',
   'fas fa-diagram-project',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_CONFIG'), 10,'LEAF','ACCOUNTS','accounts.mapping.view',TRUE, TRUE, FALSE),

  ('M_ACC_POLICY',           'Voucher Policy',             '/accounts/policy',
   'fas fa-shield-check',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_CONFIG'), 20,'LEAF','ACCOUNTS','accounts.policy.view',TRUE, TRUE, FALSE),

  ('M_ACC_AUTO_TEMPLATES',   'Auto Journal Templates',     '/accounts/auto-templates',
   'fas fa-wand-magic-sparkles',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_CONFIG'), 30,'LEAF','ACCOUNTS','accounts.autotemplate.view',TRUE, TRUE, FALSE),

  -- ── REPORTS GROUP ────────────────────────────────────────────────────────────
  ('M_ACC_REPORTS',          'Reports',                    '#',
   'fas fa-chart-bar',
   (SELECT id FROM app_menus WHERE menu_code='M_ACCOUNTS'), 40,'GROUP','ACCOUNTS',NULL,TRUE, TRUE, FALSE),

  ('M_ACC_TRIAL',            'Trial Balance',              '/accounts/reports/trial-balance',
   'fas fa-balance-scale',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 10,'LEAF','ACCOUNTS','accounts.report.trial',TRUE, TRUE, FALSE),

  ('M_ACC_PL',               'Profit & Loss',              '/accounts/reports/profit-loss',
   'fas fa-chart-line',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 20,'LEAF','ACCOUNTS','reports.pl.view',TRUE, TRUE, FALSE),

  ('M_ACC_BS',               'Balance Sheet',              '/accounts/reports/balance-sheet',
   'fas fa-file-invoice-dollar',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 30,'LEAF','ACCOUNTS','reports.bs.view',TRUE, TRUE, FALSE),

  ('M_ACC_LEDGER',           'Account Ledger',             '/accounts/reports/ledger',
   'fas fa-book',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 40,'LEAF','ACCOUNTS','accounts.report.ledger',TRUE, TRUE, FALSE),

  ('M_ACC_CASHBOOK',         'Cash Book',                  '/accounts/reports/cash-book',
   'fas fa-money-bill',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 50,'LEAF','ACCOUNTS','accounts.report.cashbook',TRUE, TRUE, FALSE),

  ('M_ACC_BANKBOOK',         'Bank Book',                  '/accounts/reports/bank-book',
   'fas fa-building-columns',
   (SELECT id FROM app_menus WHERE menu_code='M_ACC_REPORTS'), 60,'LEAF','ACCOUNTS','accounts.report.bankbook',TRUE, TRUE, FALSE)
