...
WHERE fc.code = 'FAC-PM-PROD' AND fc.organization_id = 1
  AND cc.cost_center_code = 'CC-PRD'
  AND dpt.code = 'DEPT-PRD' AND dpt.organization_id = 1
  AND wh.warehouse_code = 'WH-RM-01'
...
WHERE fc.code = 'FAC-IT-LAPTOP' AND fc.organization_id = 1
  AND cc.cost_center_code = 'CC-ACC'
  AND dpt.code = 'DEPT-ACC' AND dpt.organization_id = 1
...
WHERE fc.code = 'FAC-VEH' AND fc.organization_id = 1
  AND cc.cost_center_code = 'CC-SPN'
  AND dpt.code = 'DEPT-OPS' AND dpt.organization_id = 1
  AND wh.warehouse_code = 'WH-FG-01'
    ON CONFLICT ON CONSTRAINT uq_fa_org_code DO NOTHING;
...
SELECT DATE '2026-05-31', 'MONTHLY', DATE '2026-05-01', DATE '2026-05-31', 'POSTED', 4, 38456.78,
       'system', NOW(), NULL, 1, NOW(), NOW(), 'system', 'system'
WHERE NOT EXISTS (SELECT 1 FROM fa_depreciation_runs r WHERE r.organization_id = 1 AND r.run_date = DATE '2026-05-31' AND r.run_type = 'MONTHLY');

INSERT INTO fa_depreciation_runs
(run_date, run_type, period_start, period_end, status, total_assets, total_depreciation,
 posted_by, posted_at, journal_entry_id, organization_id, created_at, updated_at, created_by, updated_by)
SELECT DATE '2026-06-30', 'MONTHLY', DATE '2026-06-01', DATE '2026-06-30', 'DRAFT', 4, 0.00,
       NULL, NULL, NULL, 1, NOW(), NOW(), 'system', 'system'
WHERE NOT EXISTS (SELECT 1 FROM fa_depreciation_runs r WHERE r.organization_id = 1 AND r.run_date = DATE '2026-06-30' AND r.run_type = 'MONTHLY');
...
WHERE r.run_date = DATE '2026-05-31' AND r.organization_id = 1
  ...
  AND NOT EXISTS (SELECT 1 FROM fa_depreciation_run_lines l WHERE l.depreciation_run_id = r.id AND l.asset_id = a.id);
