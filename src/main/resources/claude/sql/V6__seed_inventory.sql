...
(model_code, model_name, description, is_active, organization_id, brand_id, created_at, updated_at, created_by,
 updated_by)
SELECT v.model_code,
       v.model_name,
       v.description,
...
FROM inv_item_brands b
         CROSS JOIN (VALUES ('6203-2RS', '6203-2RS', 'SKF Bearing'),
                            ...
                            ('6306-2RS', '6306-2RS', 'SKF Bearing')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('GS24', 'Galaxy S24', 'Samsung Smartphone'),
                            ...
                            ('M35', 'Galaxy M35', 'Samsung Smartphone')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('AC18INV', 'Dual Inverter 1.5 Ton', 'Air Conditioner'),
                            ('AC24INV', 'Dual Inverter 2 Ton', 'Air Conditioner'),
                            ('WM8KG', 'Front Load 8KG', 'Washing Machine'),
                            ('REF300L', '300L Refrigerator', 'Refrigerator')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('M2BAX90L', '3 HP Motor', 'ABB Motor'),
                            ('M2BAX100L', '5 HP Motor', 'ABB Motor'),
                            ('M2BAX112M', '7.5 HP Motor', 'ABB Motor')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('S71200', 'S7-1200 PLC', 'PLC'),
                            ('S71500', 'S7-1500 PLC', 'PLC'),
                            ('MM420', 'Micromaster 420', 'VFD'),
                            ('MM440', 'Micromaster 440', 'VFD')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('ATV320', 'ATV320', 'VFD'),
                            ('ATV630', 'ATV630', 'VFD'),
                            ('M221', 'Modicon M221', 'PLC'),
                            ('M241', 'Modicon M241', 'PLC')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('CP1E', 'CP1E PLC', 'PLC'),
                            ('CP1H', 'CP1H PLC', 'PLC'),
                            ('E3Z', 'E3Z Sensor', 'Sensor'),
                            ('E2E', 'E2E Sensor', 'Sensor')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('MS300', 'MS300', 'VFD'),
                            ('CP2000', 'CP2000', 'VFD'),
                            ('DVP14SS', 'DVP14SS2 PLC', 'PLC'),
                            ('DVP28SV', 'DVP28SV PLC', 'PLC')) v(model_code, model_name, description)
...
SELECT v.code,
       ...
       'system',
       'system'
FROM (VALUES ('CAT-RAW', 'Raw Materials', 'Raw material root category', 'RAW_MATERIAL'),
             ...
             ('CAT-CONS', 'Consumables', 'Consumables root category',
              'CONSUMABLE')) v(code, name, description, item_type)
...
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-RAW-FOOD', 'Food Ingredients', 'Food raw materials'),
                            ...
                            ('CAT-RAW-PAPER', 'Paper Materials', 'Paper raw materials')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-GROC', 'Grocery', 'Grocery products'),
                            ...
                            ('CAT-FG-SPORT', 'Sports & Toys', 'Sports and toy products')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-SPARE-MECH', 'Mechanical Spares', 'Mechanical spare parts'),
                            ('CAT-SPARE-ELEC', 'Electrical Spares', 'Electrical spare parts'),
                            ('CAT-SPARE-AUTO', 'Automotive Spares', 'Vehicle spare parts'),
                            ('CAT-SPARE-IT', 'IT Spares', 'Computer and IT spare parts')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-CONS-PACK', 'Packaging Consumables', 'Packaging consumables'),
                            ('CAT-CONS-CLEAN', 'Cleaning Supplies', 'Cleaning consumables'),
                            ('CAT-CONS-OFF', 'Office Supplies', 'Office consumables')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-ASSET-COMP', 'Computers', 'Computer assets'),
                            ('CAT-ASSET-FURN', 'Furniture', 'Furniture assets'),
                            ('CAT-ASSET-VEH', 'Vehicles', 'Vehicle assets'),
                            ('CAT-ASSET-MACH', 'Machinery', 'Machinery assets'),
                            ('CAT-ASSET-BUILD', 'Buildings', 'Building assets')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-SERV-REPAIR', 'Repair Service', 'Repair services'),
                            ...
                            ('CAT-SERV-MAINT', 'Maintenance Service',
                             'Maintenance services')) v(code, name, description)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-GROC-RICE', 'Rice'),
                            ...
                            ('CAT-FG-GROC-MILK', 'Milk Products')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-BEV-WATER', 'Mineral Water'),
                            ('CAT-FG-BEV-SOFT', 'Soft Drinks'),
                            ('CAT-FG-BEV-JUICE', 'Juice'),
                            ('CAT-FG-BEV-TEA', 'Tea'),
                            ('CAT-FG-BEV-COFFEE', 'Coffee'),
                            ('CAT-FG-BEV-ENERGY', 'Energy Drinks')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-ELEC-MOB', 'Mobile Phones'),
                            ...
                            ('CAT-FG-ELEC-ACC', 'Accessories')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-HOME-KITCH', 'Kitchenware'),
                            ('CAT-FG-HOME-COOK', 'Cookware'),
                            ('CAT-FG-HOME-FURN', 'Furniture'),
                            ('CAT-FG-HOME-DECO', 'Home Decor')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-STAT-PEN', 'Pens'),
                            ('CAT-FG-STAT-PAPER', 'Paper'),
                            ('CAT-FG-STAT-NOTE', 'Notebooks'),
                            ('CAT-FG-STAT-OFFICE', 'Office Supplies')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-PHAR-MED', 'Medicine'),
                            ('CAT-FG-PHAR-SURG', 'Surgical'),
                            ('CAT-FG-PHAR-HCARE', 'Healthcare'),
                            ('CAT-FG-PHAR-SUPP', 'Supplements')) v(code, name)
...
SELECT v.code,
       ...
       'system'
FROM inv_item_categories p
         CROSS JOIN (VALUES ('CAT-FG-AUTO-BATT', 'Battery'),
                            ('CAT-FG-AUTO-TYRE', 'Tyres'),
                            ('CAT-FG-AUTO-OIL', 'Lubricants'),
                            ('CAT-FG-AUTO-FLTR', 'Filters')) v(code, name)
...
-- =============================================================================
INSERT INTO inv_item_uom (code, name, symbol, category, is_base_unit, conversion_factor, active, organization_id,
                          created_at, updated_at)
VALUES ('KG', 'Kilogram', 'kg', 'WEIGHT', true, 1.000000, true, 1, NOW(), NOW()),
       ('G', 'Gram', 'g', 'WEIGHT', false, 0.001000, true, 1, NOW(), NOW()),
       ('LTR', 'Liter', 'L', 'VOLUME', true, 1.000000, true, 1, NOW(), NOW()),
       ('MTR', 'Meter', 'm', 'LENGTH', true, 1.000000, true, 1, NOW(), NOW()),
       ('BOX', 'Box', 'box', 'PACKING', true, 1.000000, true, 1, NOW(), NOW()),
       ('PCS', 'Pieces', 'pcs', 'COUNT', true, 1.000000, true, 1, NOW(), NOW()),
       ('RLL', 'Roll', 'roll', 'UNIT', true, 1.000000, true, 1, NOW(), NOW())
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('YARN-NE30', 'Cotton Yarn NE-30', 'Cotton Yarn Count NE-30'),
                            ...
                            ('COPPER-W25', 'Copper Wire 2.5mm',
                             'Copper Wire 2.5mm diameter')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('VBELT-B55', 'V-Belt B-55', 'Industrial V-Belt Section B Length 55'),
                            ...
                            ('SUB-ASM-001', 'Sub-Assembly Unit Type A',
                             'Mechanical sub-assembly unit')) v(model_code, model_name, description)
WHERE b.brand_code = 'BRD-GEN'
ON CONFLICT ON CONSTRAINT uq_model_org_brand_code DO NOTHING;
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('CB-200', 'Cutting Blade 200mm',
                             'Industrial Cutting Blade 200mm diameter')) v(model_code, model_name, description)
...
SELECT v.model_code,
       ...
       'system'
FROM inv_item_brands b
         CROSS JOIN (VALUES ('LUBE-S68', 'Lubricating Oil Shell 68', 'Shell Tellus 68 Hydraulic Oil'),
                            ...
                            ('THINNER-PT', 'Paint Thinner',
                             'General Purpose Paint Thinner')) v(model_code, model_name, description)
WHERE b.brand_code = 'BRD-GEN'
ON CONFLICT ON CONSTRAINT uq_model_org_brand_code DO NOTHING;
