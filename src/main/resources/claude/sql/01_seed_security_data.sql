...
INSERT INTO app_menus (menu_code, menu_name, menu_url, icon, parent_id, display_order, menu_type, module_name, required_permission, target, active, visible, deleted, created_at, updated_at)
SELECT 'ACC_PAYMENT',   'Payment Vouchers',  '/accounts/payments',    'fa fa-hand-holding-usd', g.id, 20, 'LEAF', 'ACCOUNTS', 'ACC.PAYMENT.VIEW', '_self', true, true, false, NOW(), NOW() FROM app_menus g WHERE g.menu_code = 'GRP_ACC_VOUCHERS' ON CONFLICT (menu_code) DO NOTHING;
INSERT INTO app_menus (menu_code, menu_name, menu_url, icon, parent_id, display_order, menu_type, module_name, required_permission, target, active, visible, deleted, created_at, updated_at)
SELECT 'ACC_RECEIPT',   'Receipt Vouchers',  '/accounts/receipts',    'fa fa-hand-holding', g.id, 30, 'LEAF', 'ACCOUNTS', 'ACC.RECEIPT.VIEW', '_self', true, true, false, NOW(), NOW() FROM app_menus g WHERE g.menu_code = 'GRP_ACC_VOUCHERS' ON CONFLICT (menu_code) DO NOTHING;
