...
public class FinancialReportService {
    ...
    public Map<String, Object> cashFlowStatement(String startDate, String endDate) {
        ...
        for (Map<String, Object> r : rows) {
            ...
            switch (section) {
                case "OPERATING"  -> { operating.add(item);  opTotal = opTotal.add(sectionAmt); }
                case "INVESTING"  -> { investing.add(item);  invTotal = invTotal.add(sectionAmt); }
                case "FINANCING"  -> { financing.add(item);  finTotal = finTotal.add(sectionAmt); }
            }
        }
