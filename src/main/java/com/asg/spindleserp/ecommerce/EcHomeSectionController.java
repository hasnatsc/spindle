 ...
 */
@Controller
@RequestMapping("/ecommerce/home-sections")
...
public class EcHomeSectionController {
    ...
    public String index(Model m) {
        m.addAttribute("activePage", "ec-home-sections");
        return "ecommerce/ec-home-section-index";
    }
    ...
    public DataTableResponse list(@RequestParam(defaultValue = "1") int draw, @RequestParam(defaultValue = "0") int start,
        ...
        return sectionService.datatableList(draw, start, length, search);
    }
    ...
    public Map<String, Object> show(@PathVariable Long id) {
        ...
        try {
            ...
            res.put("obj", Map.of("defaultData", sectionService.findById(id)));
        } catch (Exception e) {
    ...
    @PostMapping("/save")
    @ResponseBody
    public Map<String, Object> save(@RequestBody @Valid EcHomeSectionDTO dto) {
        Map<String, Object> res = new HashMap<>();
        try {
            if (dto.getId() != null) {
                sectionService.update(dto.getId(), dto);
                res.put("message", "Home section updated.");
            } else {
                sectionService.create(dto);
                res.put("message", "Home section created.");
            }
            res.put("success", true);
        } catch (Exception e) {
            res.put("success", false);
    ...
    public Map<String, Object> toggle(@PathVariable Long id) {
        Map<String, Object> res = new HashMap<>();
        try {
            EcHomeSectionDTO d = sectionService.toggleStatus(id);
            res.put("success", true);
            res.put("message", "Section " + (Boolean.TRUE.equals(d.getActive()) ? "activated" : "deactivated") + ".");
        } catch (Exception e) {
            res.put("success", false);
            res.put("message", e.getMessage());
        }
    ...
    public Map<String, Object> delete(@PathVariable Long id) {
        ...
        try {
            ...
            res.put("success", true);
            res.put("message", "Section deleted.");
        } catch (Exception e) {
            res.put("success", false);
            res.put("message", e.getMessage());
        }
